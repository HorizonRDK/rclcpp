...
class PublisherHbmem : public PublisherBase {
 public:
  ...
  virtual void post_init_setup(
    ...
    if (rclcpp::detail::resolve_use_intra_process(options_, *node_base)) {
      ...
      auto ipm =
          context->get_sub_context<rclcpp::experimental::IntraProcessManager>();
      // Register the publisher with the intra process manager.
      if (qos.get_rmw_qos_profile().history ==
          RMW_QOS_POLICY_HISTORY_KEEP_ALL) {
        throw std::invalid_argument(
            "intraprocess communication is not allowed with keep all history "
            "qos policy");
      }
      if (qos.get_rmw_qos_profile().depth == 0) {
        throw std::invalid_argument(
            "intraprocess communication is not allowed with a zero qos history "
            "depth value");
      }
      ...
      uint64_t intra_process_publisher_id =
          ipm->add_publisher(this->shared_from_this());
  ...
  //     auto shared_msg =
  //     this->do_intra_process_publish_and_return_shared(std::move(msg));
  ...
  //   }
  //   // Otherwise we have to allocate memory in a unique_ptr and pass it
  //   along.
  //   // As the message is not const, a copy should be made.
  //   // A shared_ptr<const MessageHbmem> could also be constructed here.
  //   auto ptr = MessageAllocatorTraits::allocate(*message_allocator_.get(),
  //   1); MessageAllocatorTraits::construct(*message_allocator_.get(), ptr,
  //   msg); MessageUniquePtr unique_msg(ptr, message_deleter_);
  ...
  // {
  //   return
  //   this->do_serialized_publish(&serialized_msg.get_rcl_serialized_message());
  // }
  ...
  void publish(rclcpp::LoanedHbmemMessage<MessageT, AllocatorT>&& loaned_msg) {
    ...
    this->do_loaned_message_publish(std::move(loaned_msg.release_message()));
  }
 ...
 protected:
  ...
  //     throw std::runtime_error("storing serialized messages in intra process
  //     is not supported yet");
  //   }
  //   auto status = rcl_publish_serialized_message(publisher_handle_.get(),
  //   serialized_msg, nullptr); if (RCL_RET_OK != status) {
  //     rclcpp::exceptions::throw_from_rcl_error(status, "failed to publish
  //     serialized message");
  //   }
  // }
  ...
  // void do_intra_process_publish(std::unique_ptr<MessageT, MessageDeleter>
  // msg) {
  //   auto ipm = weak_ipm_.lock();
  //   if (!ipm) {
  //     throw std::runtime_error(
  //         "intra process publish called after destruction of intra process "
  //         "manager");
  //   }
  //   if (!msg) {
  ...
  // do_intra_process_publish_and_return_shared(std::unique_ptr<MessageT,
  // MessageDeleter> msg)
  // {
  //   auto ipm = weak_ipm_.lock();
  //   if (!ipm) {
  //     throw std::runtime_error(
  //             "intra process publish called after destruction of intra
  //             process manager");
  //   }
  //   if (!msg) {
  //     throw std::runtime_error("cannot publish msg which is a null pointer");
  ...
  //   return ipm->template
  //   do_intra_process_publish_and_return_shared<MessageHbmem, AllocatorT>(
